...
| Conversion | Pure JavaScript (fflate) |
...
2. **Create the Worker**
   ...
   2. Name it `telegram-bot-subtitle-converter` and click **Deploy** (you'll land in the code editor).
...
4. **Point Telegram at your bot** — open this URL in your browser (use your Worker's real URL, shown in the dashboard; `<subdomain>` is your account's `*.workers.dev` subdomain):
   ```
   https://telegram-bot-subtitle-converter.<subdomain>.workers.dev/register
   ```
...
| --- | --- |
| `/srt2vtt` <srt> | SRT to VTT |
| `/vtt2srt` <vtt> | VTT to SRT |
| `/help` | Show help |
